...
export function Stake() {
  ...
  async function getAmountOut() {
    ...
    try {
      ...
      const amountOfTokensToBuy = ethers.utils.parseUnits(
        userStakingBalance,
        18
      );
      ...
      const FormattedwBNBTokenAmount = ethers.utils.formatUnits(
        wBNBTokenAmount,
        18
      );
      const FormattedTopTokenAmount = ethers.utils.formatUnits(
        topTokenAmount,
        18
      );
  ...
  const HandlePremiumEligible = async (): Promise<void> => {
    ...
    try {
      if (userStakingBalance == "0.000") {
        ...
        const tx = await contract.checkPremuimEligible(
          bnbworth,
          topTokenAmount,
          address
        );
  ...
  return (
    ...
    <div style={{ position: 'relative' }}>
      ...
      <div style={isConnected && chainId !== 56 ? { filter: "blur(8px)" } : {}}>
        <Tabs defaultValue="account" className="w-[350px] md:w-[500px]">
              ...
              <CardHeader>
                ...
                <CardDescription>
                  <div className="mt-2 text-xs md:text-sm">
                    Stake your $TOP tokens now to earn juicy rewards at 10% APY!
                  </div>
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-2 ">
                <div className="flex w-full justify-between space-x-2 font-mono text-sm md:text-base">
                  <h1 className="">Your Rewards: {userReward} $TOP</h1>
                  <h1>My balance: {userBalance} $TOP</h1>
                </div>
                <div className="border-2 rounded-sm text-base md:text-lg">
                  Staked $TOP: {userStakingBalance}
                </div>
                <div className="border-2 rounded-xs text-base md:text-lg">
                ...
                <div className="flex w-full  items-center space-x-1 mt-2 ">
                  <Input
                    ...
                    placeholder="Amount of $TOP tokens..."
                  />
                  <Button onClick={handleMaxStake}>Max</Button>
                </div>
                <div className="space-y-1 flex flex-start font-thin font-mono text-sm">
                  <h1 className="mt-2 text-xs md:text-base">
                    Stake $100 worth of $TOP to be eligible for premium features
                    on our App!
                  </h1>
                  <button
                    className="border-2 rounded-sm hover:bg-blue-500 hover:text-white text-xs px-2 md:text-base"
                    onClick={() => HandlePremiumEligible()}
                  >
                    Check Premium
                  </button>
                </div>
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex w-full  items-center space-x-1">
                  <Input
                    ...
                    placeholder="Withdraw From your Staked $TOP balance."
                  />
                  <Button onClick={handleMaxUnstake}>Max</Button>
                </div>
                <div className="space-y-1 flex flex-start font-thin font-mono">
                  <h1 className="mt-2 text-xs p-2 md:text-base ">
                    Stake $100 worth of $TOP to be eligible for premium features
                    on our App!
                  </h1>
                  <button
                    className="border-2 rounded-sm hover:bg-blue-500 hover:text-white px-2 text-xs md:text-base"
                    onClick={() => HandlePremiumEligible()}
                  >
                    Check Premium
                  </button>
